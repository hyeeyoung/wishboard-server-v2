...
public class TokenService {
	...
	@Transactional
	public TokenResponseDto getRefreshToken(TokenCommand tokenCommand, String deviceInfo) {
		if (!jwtProvider.validateToken(tokenCommand.getRefreshToken())) {
			throw new UnAuthorizedException(String.format("주어진 리프레시 토큰 (%s) 이 유효하지 않습니다.", tokenCommand.getRefreshToken()), INVALID_TOKEN);
		}
		Long userId = jwtProvider.getUserIdFromJwt(tokenCommand.getRefreshToken());
		...
		if (Objects.isNull(refreshToken)) {
			throw new UnAuthorizedException(String.format("이미 만료된 리프레시 토큰 (%s) 입니다.", tokenCommand.getRefreshToken()), TOKEN_EXPIRED);
		}
		if (!refreshToken.equals(tokenCommand.getRefreshToken())) {
			throw new UnAuthorizedException(String.format("해당 리프레시 토큰의 정보 (%s) 가 일치하지 않습니다.", tokenCommand.getRefreshToken()), INVALID_TOKEN);
		}
		return jwtProvider.createTokenInfo(userId, deviceInfo);
	}
