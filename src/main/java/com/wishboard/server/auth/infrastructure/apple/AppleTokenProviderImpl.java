...
import com.wishboard.server.auth.infrastructure.apple.dto.response.ApplePublicKeyResponse;
...
public class AppleTokenProviderImpl implements AppleTokenProvider {
	...
	public String getSocialIdFromIdToken(String idToken) {
		...
		try {
			Map<String, String> header = objectMapper.readValue(new String(Base64.getDecoder().decode(headerIdToken), StandardCharsets.UTF_8),
				new TypeReference<>() {
				});
	...
	private PublicKey getPublicKey(Map<String, String> header) throws InvalidKeySpecException, NoSuchAlgorithmException {
		ApplePublicKeyResponse response = appleApiCaller.getAppleAuthPublicKey().block();
